    ...
    hi_syserr_info *sysErrInfo)
{
    ...
    if (filePath == NULL || errInfo == NULL || sysErrInfo == NULL){
        BBOX_PRINT_ERR("filePath: %p, errInfo: %p, sysErrInfo: %p!\r\n",
    ...
    if (len > 0) {
        ...
        BBOX_PRINT_ERR("snprintf_s failed or the buffer is too short!\r\n");
    }
}
...
static void SaveExcpInfo(const char *filePath, hi_syserr_info *sysErrInfo)
{
    ...
    if (filePath == NULL || sysErrInfo == NULL) {
        BBOX_PRINT_ERR("filePath: %p, sysErrInfo: %p\r\n",
            filePath, sysErrInfo);
        return;
    }
...
static void SaveCallBack(const char *filePath, hi_syserr_info *sysErrInfo)
{
    ...
    if (hi_get_crash_partition_table(&addr, &size) != 0) {
        BBOX_PRINT_ERR("hi_get_crash_partition_table failed!\r\n");
        return;
    }

    pData = hi_malloc(HI_MOD_ID_SAL_DFX, stackSize);
    if (pData == NULL) {
        BBOX_PRINT_ERR("hi_malloc failed!\r\n");
        return;
    }

    if (hi_flash_read(addr + size - stackSize, stackSize,
        (hi_u8 *)pData) != HI_ERR_SUCCESS) {
        BBOX_PRINT_ERR("Read stack from crash partition failed!\r\n");
...
static void SaveFaultLog(struct ErrorInfo *errInfo, hi_syserr_info *sysErrInfo)
{
    if (errInfo == NULL || sysErrInfo == NULL) {
        BBOX_PRINT_ERR("errInfo: %p, sysErrInfo: %p!\r\n",
            errInfo, sysErrInfo);
        return;
    }
...
static void SetLogSavedFlag(hi_void)
{
    ...
    if (hi_flash_write(addr, sizeof(LOG_SAVED_FLAG), 
        (hi_u8 *)LOG_SAVED_FLAG, HI_TRUE) != HI_ERR_SUCCESS) {
            BBOX_PRINT_ERR("Set log saved flag failed!\r\n");
        }
}
...
static void Dump(const char *logDir, struct ErrorInfo *info)
{
    ...
    if (len <= 0) {
        BBOX_PRINT_ERR("buf isn't enough or vsnprintf_s failed\r\n");
        hi_free(HI_MOD_ID_SAL_DFX, buf);
        return;
    }
    (VOID)FullWriteFile(FAULT_LOG_PATH, buf, len, 0);
    hi_free(HI_MOD_ID_SAL_DFX, buf);
}
...
static int GetLastLogInfo(struct ErrorInfo *info)
{
    ...
    if (ret != HI_ERR_SUCCESS) {
        BBOX_PRINT_ERR("No crash dump found!\r\n");
    ...
    if (ret == HI_ERR_SUCCESS) {
        BBOX_PRINT_INFO("latest reboot reason:\r\neid:%d,"
            " rid:%d:\r\n", err_id, rid);
    }
...
static void SystemAdapterInit(void)
{
    ...
    BBoxNotifyError("EVENT_TEST", MODULE_SYSTEM, "Test BBoxNotifyError!", 0);
#endif
}
